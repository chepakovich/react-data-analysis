...
const BasicMap = (props) => {
  ...
  const { eventsData } = props.stateData
  ...
  eventsData.forEach(
    function(d){
        ...
        markers2[d.poi_id] += parseFloat(d.events)
    }
  )

  let markers3 = []
  markers2.forEach(function (value, i) {
    {markers3[i-1] = { name: places[i-1].name, coordinates: places[i-1].coordinates, population: value*800000}}
  })
